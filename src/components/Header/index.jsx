...
const Header = ({ active }) => {
    ...
    return (
        <div className='w-[90%] flex md:flex-row justify-between items-center flex-wrap'>
            <div>
                <img src={logo} alt="my logo" />
            </div>
            <div className={`w-full md:w-auto order-4 md:order-none md:flex mt-3 ${isOpen ? 'flex' : 'hidden'} flex-col md:flex-row space-y-4 md:space-y-0 space-x-0 md:space-x-[35px] lg:space-x-20 transition ease-in duration-1000 delay-1000`}>
                {nav.map((item, index) => (
